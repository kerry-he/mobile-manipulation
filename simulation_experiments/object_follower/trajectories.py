...
def get_vals_from_constants(consts, t, func=numpy.sin):
    ...
    freq_mult = numpy.arange(
        1, term_count + 1).astype(numpy.float32).reshape(-1, 1) * 2 * numpy.pi
...
class SplineY(TrajectoryShape):
    ...
    def generate(self, t_count=1000):
        ...
        t = numpy.linspace(0, 1, t_count)
        key_points = numpy.zeros(
            (self.n_key_points + 2, 2), dtype=numpy.float32)
        ...
        while True:
            ...
            spline_consts = splrep(
                key_points[:, 0], key_points[:, 1], s=0, k=3)
...
class RandomWalk(TrajectoryShape):
    ...
    def generate(self, t_count=1000):
        points = numpy.zeros(
            (self.n_direction_changes, 2), dtype=numpy.float32)
        ...
        for i in range(1, self.n_direction_changes):
            while 1:
                ...
                points[i, :] = points[i-1, :] + numpy.array(
                    [numpy.cos(angle), numpy.sin(angle)], dtype=numpy.float32) * dist
...
def calculate_radius_points(angle, prev_angle, prev_point, radius):
    ...
    center = prev_point + numpy.array([numpy.cos(center_angle_rad), numpy.sin(
        center_angle_rad)], dtype=numpy.float32) * radius

    # print(angle)
    inv_center_angle = center_angle + 180
    theta = numpy.deg2rad(numpy.linspace(
        inv_center_angle, inv_center_angle + angle, 50))

    new_corner_points = numpy.vstack([numpy.cos(theta), numpy.sin(theta)]).astype(
        numpy.float32).T * radius + center
...
def interpolate_trajectory(traj, max_distance=0.01):
    ...
    for i in range(1, traj.shape[0]):
        ...
        if dist_to_previous > max_distance:
            ...
            vals = numpy.tile(numpy.linspace(
                0, 1, n_required + 1)[1:].reshape(-1, 1), 2)
            # print(n_required, vals)
            # print(traj[i, :]* vals)
            new_traj = numpy.vstack(
                [new_traj, (new_traj[-1, :] * (1-vals) + traj[i, :] * vals)])
    ...
    def generate(self, t_count=1000):
        ...
        for i in range(1, self.n_direction_changes):
            ...
            while 1:
                ...
                if prev_angle is not None:
                    new_corner_points = calculate_radius_points(
                        angle, prev_angle, points[-1, :], self.radius)
                    corner_points_out_of_bounds = numpy.any(
                        numpy.abs(new_corner_points) > 1)

                    angle += prev_angle

                angle = numpy.deg2rad(angle)

                new_point_offset = numpy.array(
                    [numpy.cos(angle), numpy.sin(angle)], dtype=numpy.float32) * dist
                ...
                if new_corner_points is not None:
                    next_corner_points = calculate_radius_points(
                        360, numpy.rad2deg(angle), new_point, self.radius)
                    next_corner_points_in_bounds_1 = numpy.all(
                        numpy.abs(next_corner_points) < 1)

                    next_corner_points = calculate_radius_points(
                        -360, numpy.rad2deg(angle), new_point, self.radius)
                    # print(new_point)
                    # print(next_corner_points)
                    next_corner_points_in_bounds_2 = numpy.all(
                        numpy.abs(next_corner_points) < 1)
...
def transform_trajectory(traj, matrix):
    traj_vector = numpy.insert(traj, traj.shape[1], 1, axis=1)
    transformed_traj = traj_vector @ matrix.T
    return transformed_traj[:, :2]
...
if __name__ == "__main__":
    ...
    for i, t in enumerate(TRAJECTORIES):
        ...
        if max_val <= 1 and min_val >= -1:
            ...
            traj = scale_trajectory(
                traj, workspace_bounds[0], workspace_bounds[1])  # Max 140, 140

...
class Ours(BaseController):
    ...
    def step(self, panda, Tep, NUM_OBJECTS, n, collisions, table, index, wTcamp, wTtp):
        ...
        occluded, Ain, bin = self.calcVelocityDamper(panda, collisions, NUM_OBJECTS, n, Ain, bin, wTcamp, wTtp)
    ...
    #     # Slack component of Q
    #     Q[n : n + 6, n : n + 6] = pos_w * (1 / np.power(et, pos_p)) * np.eye(6)

    #     # 4th is equality
    #     Q[n + 6 : n + 7, n + 6 : n + 7] = rot_w * (1 / (np.power(et, rot_p))) * np.eye(1)
    ...
    #     Aeq = np.c_[panda.jacobe(panda.q), np.eye(6), np.zeros((6, 1 + NUM_OBJECTS))]
    #     beq = v.reshape((6,))
    ...
    #     # The lower and upper bounds on the joint velocity and slack variable
    #     lb = -np.r_[panda.qdlim[:n]*1/5, 10 * np.ones(6), 10 * np.ones(1), np.zeros(N_SLACK_TERMS -7)]
    #     # lb = -np.r_[panda.qdlim[:n], 10 * np.ones(N_SLACK_TERMS)]
    ...
    #     c_Ain = np.c_[
    #         c_Ain, np.zeros((c_Ain.shape[0], 7)), slack_matrix
    #     ]

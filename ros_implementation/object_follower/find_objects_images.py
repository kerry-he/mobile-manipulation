...
class image_converter:
  ...
  def __init__(self):
    ...
    # self.depth_sub = rospy.Subscriber("/depth_registered/image",Image,self.depth_callback,queue_size=1)

    self.camera_info_sub = rospy.Subscriber("/camera/rgb/camera_info", CameraInfo, self.cameraCallback, queue_size=1)
...
#       self.depth_image = self.bridge.imgmsg_to_cv2(data, "passthrough")
  ...
  def callback(self,data):
    ...
    if len(contour)>0:
        ...
        if radius > 1:
            cv2.circle(cv_image, (int(self.x), int(self.y)), int(radius), colors, 2)
            cv2.putText(cv_image, "OBJECT", (int(self.x - (radius+1)), int(self.y - (radius+1))), cv2.FONT_HERSHEY_COMPLEX, 0.8, colors, 2)
    ...
    world_x, world_y, world_z = np.matmul(self.cameraMatrixInv, np.array([self.x,self.y,1])) * self.appleHeight

...
def step_robot_base(r, r_cam, Tep):

    # wTe = r.fkine(r.q, fast=True)
    # wTe = r._base.A[:] / 2 + r.fkine(r.q, fast=True) / 2
    wTe = r._base.A[:] @ sm.SE3(1.1281/2, 0, 0).A
...
def step_robot_arm(r, r_cam, Tep):
...
for goal in range(30):
    ...
    while not arrived:
        ...
        # try:
        arrived, fetch.qd, fetch_camera.qd = step_robot_base(fetch, fetch_camera, wTep.A)
    ...
    while not arrived:
        ...
        arrived, fetch.qd, fetch_camera.qd = step_robot_arm(fetch, fetch_camera, wTep.A)
        # except Exception as e:
        #     print(e)
        #     break
    ...
    with open('separate_tucked_data.csv', 'a', newline='') as csvfile:

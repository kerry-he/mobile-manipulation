...
def step_robot(r, r_cam, Tep):
    ...
    # Q[-8, -8] *= 1000
    Q[r.n + 5 : -4, r.n + 5 : -4] = (1.0 / (et * et * et * et)) * np.eye(3)      # Slack manipulator
    Q[-4:-1, -4:-1] = 10 * np.eye(3)                                # Slack camera
    Q[-1, -1] = et
    ...
    global seen_count
    global total_count
    if min(c_din) > 0.05 and head_angle < np.deg2rad(45):
        seen_count += 1
    total_count += 1
    ...
    #     end=r.link_dict["wrist_flex_link"],
    # )

    # If there are any parts of the robot within the influence distance
    # to the collision in the scene
    # if c_Ain is not None and c_bin is not None:
    #     c_Ain = np.c_[c_Ain, np.zeros((c_Ain.shape[0], 2 + 10))]
...
# Table
# table = sg.Cuboid(scale=(1.0, 1.0, 1.0), base=sm.SE3(wTep.A[0, -1] - 0.4, wTep.A[1, -1],  wTep.A[2, -1]/2 - 0.2))
# env.add(table)

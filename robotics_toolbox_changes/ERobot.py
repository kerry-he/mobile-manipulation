...
class ERobot(BaseERobot):
    ...
    def new_vision_collision_damper(
        ...
        def indiv_calculation(link, link_col, q, min_dist, min_wTc):
            d, wTlp, wTcp = link_col.closest_point(shape, di)
            ...
            if d is not None:
                ...
                wTc = camera.fkine(camera.q, fast=True)
                total_length = np.linalg.norm(wTc[:3, 3] - obj)
                ...
                if d < 0:
                    dist = np.linalg.norm(wTlp - wTc[:3, 3])
            ...
            return l_Ain, l_bin, d, min_dist, min_wTc
        ...
        for link in links:
            ...
            for link_col in col_list:
                l_Ain, l_bin, d, min_dist, min_wTc = indiv_calculation(link, link_col, q, min_dist, min_wTc)
